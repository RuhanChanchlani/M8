...
<div className="hidden md:flex items-center gap-10 font-['Manrope'] font-medium tracking-tight">
  {user && (user.role === 'staff' || user.role === 'manager') && (
    <Link className="text-[#765700] hover:text-[#91472a] transition-colors" href="/responder">Responder View</Link>
  )}
  {user && user.role === 'manager' && (
    <Link className="text-[#91472a] font-bold border-b-2 border-[#91472a] pb-1" href="/command">Command Center</Link>
  )}
  {user && user.role === 'guest' && (
    <Link className="text-[#765700] hover:text-[#91472a] transition-colors" href="/guest/sos">Guest SOS</Link>
  )}
</div>
<div className="flex items-center gap-6">
  {user ? (
    <>
      <span className="text-sm font-bold text-on-surface-variant">Hello, {user.name || user.id}</span>
      <button 
        onClick={logout}
        className="text-[#91472a] font-bold hover:text-[#765700] transition-colors"
      >
        Logout
      </button>
    </>
  ) : (
    <Link href="/login" className="text-[#91472a] font-bold hover:text-[#765700] transition-colors">Login</Link>
  )}
  <Link href={user ? (user.role === 'guest' ? '/guest/sos' : '/responder') : '/login'} className="text-on-primary px-6 py-2.5 rounded-full font-bold text-sm tracking-wide liquid-glass hover:brightness-110 transition-all flex items-center justify-center">
    {user ? (user.role === 'guest' ? 'Emergency Alert' : 'Responder Hub') : 'Get Started'}
  </Link>
</div>
</nav>
...
<div className="flex items-center gap-6">
  <Link href={user ? (user.role === 'manager' ? '/command' : (user.role === 'staff' ? '/responder' : '/guest/sos')) : '/login'} className="liquid-glass text-on-primary px-10 py-5 rounded-full text-lg font-bold shadow-2xl hover:brightness-110 transition-all flex items-center gap-3">
                        {user ? 'Open Dashboard' : 'Launch Command'}
                        <span className="material-symbols-outlined" data-icon="arrow_forward">arrow_forward</span>
  </Link>
  {!user && (
    <Link href="/login" className="heavy-glass px-10 py-5 rounded-full text-lg font-bold text-primary hover:bg-white/40 transition-all border border-white/60 flex items-center justify-center">
        Responder Hub
    </Link>
  )}
</div>
...
<Link href={user && user.role === 'guest' ? "/guest/sos" : "/login"} className="md:col-span-7 heavy-glass rounded-[3rem] p-12 flex flex-col justify-between min-h-[450px] group hover:shadow-2xl transition-all duration-500 border border-white/50 block cursor-pointer">
<div>
<div className="bg-primary w-16 h-16 rounded-2xl flex items-center justify-center mb-8 shadow-inner group-hover:scale-110 transition-transform">
<span className="material-symbols-outlined text-white text-3xl" data-icon="emergency">emergency</span>
</div>
<h3 className="text-4xl font-bold text-on-surface mb-6 group-hover:text-primary transition-colors">Guest SOS</h3>
...
<Link href={user && (user.role === 'staff' || user.role === 'manager') ? "/responder" : "/login"} className="md:col-span-4 heavy-glass rounded-[3rem] p-10 flex flex-col justify-center border-b-8 border-primary border-t-white/50 border-x-white/50 group hover:shadow-2xl transition-all duration-500 cursor-pointer block">
<span className="material-symbols-outlined text-primary text-5xl mb-6" data-icon="hub" data-weight="fill">hub</span>
<h4 className="text-2xl font-bold mb-4 group-hover:text-primary transition-colors">Responder Hub</h4>
<p className="text-on-surface-variant font-medium leading-relaxed">
                            A unified tactical interface for field staff. Real-time GPS mapping and instant biometric feedback.
                        </p>
</Link>

<Link href={user && user.role === 'manager' ? "/command" : "/login"} className="md:col-span-8 heavy-glass rounded-[3rem] p-10 flex items-center gap-12 border border-white/50 group hover:shadow-2xl transition-all duration-500 cursor-pointer block">
<div className="flex-1">
<h4 className="text-3xl font-bold mb-4 group-hover:text-primary transition-colors">Admin Governance</h4>
...
<div className="hidden lg:block w-1/3 aspect-square rounded-full border-8 border-primary/20 flex items-center justify-center p-8 bg-white/10 group-hover:bg-white/20 transition-all">
...
<section className="px-12">
<div className="max-w-7xl mx-auto heavy-glass rounded-[3rem] p-16 flex flex-col md:flex-row justify-between items-center gap-12 border border-white/50">
...
<div className="grid grid-cols-2 gap-12 w-full md:w-auto text-center md:text-right">
<div className="flex flex-col items-center md:items-end">
<div className="text-5xl font-bold text-primary mb-2">99.99</div>
<div className="text-xs uppercase tracking-widest text-tertiary font-bold text-center md:text-right">Uptime Reliability</div>
</div>
<div className="flex flex-col items-center md:items-end">
<div className="text-5xl font-bold text-primary mb-2">&lt;2s</div>
<div className="text-xs uppercase tracking-widest text-tertiary font-bold text-center md:text-right">Dispatch Latency</div>
</div>
</div>
</div>
</section>
